...
def process_trace_file(input_file, output_file):
    ...
    with open(input_file, 'r') as infile:
        ...
        for line in infile:
            ...
            # Append to the list
            delayed_packets.append((delayed_time, parts[1]))

    # Sort packets by the delayed time
    delayed_packets.sort(key=lambda x: x[0])

    with open(output_file, 'w') as outfile:
        for delayed_time, direction in delayed_packets:
            # Write the modified time and original direction to the output file
            outfile.write(f"{delayed_time}\t{direction}\n")
