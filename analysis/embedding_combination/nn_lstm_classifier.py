...
train_dataset = np.load('fv_train_shortened.npy')
train_labels = np.load('fv_labels_train_shortened.npy')

#x, 11, 64
test_dataset = np.load('fv_test_shortened.npy')
test_labels = np.load('fv_labels_test_shortened.npy')
...
torch.save(model.state_dict(), 'lstm_3.pth')
model.load_state_dict(torch.load('lstm_3.pth'))

model.eval()
test_loss = 0.0
test_accuracy = 0.0
thresholds = [.01, .05, .1, .25, .3, .4, .5, .6, .7, .75, .8, .9, .95, .99,.995, .999]
